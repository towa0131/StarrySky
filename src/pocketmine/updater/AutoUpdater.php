...
class AutoUpdater{
	...
	public function showConsoleUpdate(){
		...
		$logger->warning("§3----- §eStarrySky Auto Updater §3-----§r");
	...
	public function showPlayerUpdate(Player $player){
		$player->sendMessage(TextFormat::DARK_PURPLE . "§3The version of §eStarrySky §3that this server is running is out of date. Please consider updating to the latest version.§r");
	...
	protected function showChannelSuggestionStable(){
		$logger = $this->server->getLogger();
		$logger->info("§3----- §eStarrySky Auto Updater §3-----§r");
